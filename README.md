...
1. What is the purpose of using _sessions_? Sessons allows the server to remember users in memory so that it can give apporiate data/proccess to each user and improve security.
1. What does bcrypt do to help us store passwords in a secure manner. it one way hashes passwords so that they cant be unhashed into raw form.
1. What does bcrypt do to slow down attackers? it prvents rainbow attacks by adding a unique salt to each hash.
1. What are the three parts of the JSON Web Token? options, secret, and payload.
